...
// SetIconName sets the icon name for the StatusNotifierItem.
// The name is looked up in the current icon theme. Call before
// starting the tray to set the initial value, or after to update it live.
func SetIconName(name string) {
	...
	notifier.Emit(conn, &notifier.StatusNotifierItem_NewIconSignal{
		Path: path,
		Body: &notifier.StatusNotifierItem_NewIconSignalBody{},
	})
}

// SetIconThemePath sets a custom icon theme path for the StatusNotifierItem.
// This allows the DE to find icons in a non-standard location.
func SetIconThemePath(themePath string) {
	...
	notifier.Emit(conn, &notifier.StatusNotifierItem_NewIconThemePathSignal{
		Path: path,
...
type tray struct {
	...
	// icon name and theme path for freedesktop icon lookup
